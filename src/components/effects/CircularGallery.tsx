...
// ─── Adapted from ReactBits <CircularGallery /> (ogl). Converted to strict TS:
// every field/param is typed, the dead `renderer` threading was dropped, and the
// 2D context is null-guarded. Behaviour is otherwise faithful to the original. ──
...
function loadCustomFont(fontUrl: string): Promise<string> {
  const isStylesheet =
  ...
  try {
    ...
    if (document.fonts?.load) {
      try {
        ...
        /* still attempt to render with the requested font */
      }
    }
    return resolved;
  } catch (error) {
...
class Media {
  ...
  update(scroll: ScrollState, direction: Direction): void {
  ...
  onResize(sizes?: { screen?: ScreenSize; viewport?: ViewportSize }): void {
...
class App {
  ...
  onWheel(e: Event): void {
    ...
    const delta = we.deltaY || (e as unknown as { detail: number }).detail || 0;
